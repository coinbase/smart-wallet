...
contract CoinbaseSmartWalletFactoryTest is Test {
    ...
    function test_createAccountSetsOwnersCorrectly() public {
        vm.expectCall(address(account), abi.encodeCall(CoinbaseSmartWallet.initialize, (owners)));
    ...
    function test_revertsIfNoOwners() public {
        ...
        factory.createAccount{value: 1e18}(owners, 0, address(account));
    }

    function test_exitIfAccountIsAlreadyInitialized() public {
        CoinbaseSmartWallet a = factory.createAccount(owners, 0, address(account));
        vm.expectCall(address(a), abi.encodeCall(CoinbaseSmartWallet.initialize, (owners)), 0);
        CoinbaseSmartWallet a2 = factory.createAccount(owners, 0, address(account));
    ...
    function test_initCodeHash() public {
        bytes32 execptedHash = LibClone.initCodeHashERC1967(factory.emptyImplementation());
