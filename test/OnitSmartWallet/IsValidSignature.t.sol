...
contract TestIsValidSignature is SmartWalletTestBase {
    ...
    function testSmartWalletSigner() public {
        MockOnitSmartWallet otherAccount = new MockOnitSmartWallet();
        ...
        OnitSmartWallet.SignatureWrapper memory wrapperForOtherAccount = OnitSmartWallet.SignatureWrapper(0, signature);

        bytes memory sig = abi.encode(
            OnitSmartWallet.SignatureWrapper({ownerIndex: 2, signatureData: abi.encode(wrapperForOtherAccount)})
        );
    ...
    function testValidateSignatureWithEOASigner() public {
        ...
        bytes memory signature = abi.encodePacked(r, s, v);
        bytes4 ret = account.isValidSignature(hash, abi.encode(OnitSmartWallet.SignatureWrapper(0, signature)));
        assertEq(ret, bytes4(0x1626ba7e));
    }
    ...
    function testRevertsIfEthereumSignatureButWrongOwnerLength() public {
        ...
        bytes memory signature = abi.encodePacked(r, s, v);
        vm.expectRevert();
        account.isValidSignature(hash, abi.encode(OnitSmartWallet.SignatureWrapper(1, signature)));
    }

    /// @dev this case should not be possible, but we need to explicitly test the revert case
    /// ! TODO THIS TEST IS NOT REVERTING AS EXPECTED
    function testRevertsIfOwnerIsInvalidEthereumAddress() public {
        ...
        account.isValidSignature(hash, abi.encode(OnitSmartWallet.SignatureWrapper(0, signature)));
    }
