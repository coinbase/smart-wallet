...
contract TestExecuteWithoutChainIdValidation is SmartWalletTestBase {
    ...
    function setUp() public override {
        ...
        userOpCalldata = abi.encodeWithSelector(OnitSmartWallet.executeWithoutChainIdValidation.selector);
    }
    ...
    function test_succeeds_whenSelectorAllowed() public {
        ...
        userOpCalldata = abi.encodeWithSelector(OnitSmartWallet.executeWithoutChainIdValidation.selector, calls);
    ...
    function test_reverts_whenSelectorNotApproved() public {
        bytes4 selector = OnitSmartWallet.execute.selector;
        ...
        vm.expectRevert(
            abi.encodeWithSelector(OnitSmartWallet.SelectorNotAllowed.selector, OnitSmartWallet.execute.selector)
        );
        account.executeWithoutChainIdValidation(calls);
    }

    function test_reverts_whenOneSelectorNotApproved() public {
        bytes4 badSelector = OnitSmartWallet.execute.selector;
    ...
    function _sign(UserOperation memory userOp) internal view override returns (bytes memory signature) {
        ...
        signature = abi.encode(OnitSmartWallet.SignatureWrapper(0, abi.encodePacked(r, s, v)));
    }
}
