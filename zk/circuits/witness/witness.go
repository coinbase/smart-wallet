...
func GenerateWitness[RSAFieldParams emulated.FieldParams](
	...
	ephPublicKeyAsElements, err := utils.BytesTo31BytesElements(ephPubKey)
	if err != nil {
		return
	}
	...
	witnessPublicHash = hashPublicInputs[RSAFieldParams](idpPublicKeyNBytes, ephPublicKeyAsElements, jwtHeaderJson, string(jwtHeader["kid"]))

	return
}

// func extractJsonValues(json string) (values map[string]json.RawMessage, err error) {
// 	var header map[string]json.R
// 	if err = json.Unmarshal([]byte(json), &values); err != nil {
// 		return
// 	}
